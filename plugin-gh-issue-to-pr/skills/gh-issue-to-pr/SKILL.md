...
Take `$ARGUMENTS` as the issue reference (number, URL, or free-text description) and hand off to
the `gh-issue-to-pr` agent via the Agent tool, passing the reference through as the prompt. Run
it in the foreground — the user is waiting on the next step (investigation, then a plan to
confirm), not background work.
...
- Never substitute this skill's own judgment for the agent's — no reimplementing the phases
