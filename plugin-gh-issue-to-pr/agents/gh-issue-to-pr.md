...
Before anything else, read the target repo's CLAUDE.md / AGENTS.md / CONTRIBUTING.md / docs/*POLICY* if present — its conventions (commit style, branch model, Docker-vs-local test execution, `gh` CLI vs GitHub MCP tools) override every default in this prompt. If the repo has no such docs, fall back to what's below.
